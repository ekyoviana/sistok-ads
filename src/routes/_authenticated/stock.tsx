...
function Stock() {
  ...
  const { data: products = [] } = useQuery({
    queryKey: ["products"],
    queryFn: async () =>
      (await supabase.from("products").select("*").order("nama_barang")).data ?? [],
  });
  const filtered = useMemo(
    () =>
      products.filter(
        (p: any) =>
          !q ||
          p.nama_barang.toLowerCase().includes(q.toLowerCase()) ||
          p.kode_barang.toLowerCase().includes(q.toLowerCase()),
      ),
    [products, q],
  );
  ...
  return (
    <div>
      <PageHeader title={t("stock")} subtitle={t("current_stock_snapshot")} />
          ...
          {filtered.map((p: any) => {
            ...
            return (
              <tr key={p.id}>
                ...
                <Td>{p.kategori || "-"}</Td>
                <Td>
                  <span className={`tabular-nums ${low ? "text-destructive font-semibold" : ""}`}>
                    {p.stok} {p.satuan}
                  </span>
                </Td>
                ...
                <Td>
                  {p.tanggal_kadaluarsa ? (
                    <Badge
                      variant={
                        exp === "critical" ? "destructive" : exp === "warn" ? "warning" : "muted"
                      }
                    >
                      {fmtDate(p.tanggal_kadaluarsa)}
                    </Badge>
                  ) : (
                    "-"
                  )}
