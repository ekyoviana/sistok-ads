...
function Adj() {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.rpc("fn_penyesuaian", {
        p_product_id: form.product_id,
        p_jenis: form.jenis,
        p_jumlah: form.jumlah,
        p_keterangan: form.keterangan,
      });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success(t("success_saved"));
      setForm({ product_id: "", jenis: "kerusakan", jumlah: -1, keterangan: "" });
  ...
  return (
    <div>
      <PageHeader
        title={t("adjustments")}
        subtitle={t("use_negative_numbers_for_stock_reductions")}
      />
        ...
        <tbody>
          ...
          {rows.map((r: any) => (
            <tr key={r.id}>
              ...
              <Td className={`tabular-nums ${r.jumlah < 0 ? "text-destructive" : "text-success"}`}>
                {r.jumlah > 0 ? "+" : ""}
                {r.jumlah}
              </Td>
