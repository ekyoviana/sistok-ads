...
function Reports() {
  ...
  const stockValue = products.reduce(
    (s: number, p: any) => s + (p.stok || 0) * (p.harga_jual || 0),
    0,
  );
    ...
    <div>
        ...
        actions={
          <>
            ...
            <Button onClick={handleExport}>
              <FileDown className="size-4" /> {t("export_pdf")}
            </Button>
          </>
        }
      />
      ...
      {type !== "stock" && (
        <div className="text-sm text-muted-foreground mb-2">
          Menampilkan {type === "in" ? (inQ.data?.length ?? 0) : (outQ.data?.length ?? 0)} transaksi
          · {from} → {to}
        </div>
      )}
            ...
            <tbody>
              {products.map((p: any) => (
                <tr key={p.id}>
                  ...
                  <Td>{p.nama_barang}</Td>
                  <Td>{p.kategori || "-"}</Td>
                  <Td className="tabular-nums">{p.stok}</Td>
            ...
            <tbody>
              ...
              {inQ.data?.map((r: any) => (
                <tr key={r.id}>
                  ...
                  <Td className="tabular-nums font-semibold">{fmtIDR(r.jumlah * r.harga_beli)}</Td>
                  <Td>{r.supplier || "-"}</Td>
                </tr>
              ))}
            </tbody>
