  ...
  const { data: outToday = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("outgoing_goods")
        .select("id")
        .gte("tanggal_keluar", start.toISOString());
      return data ?? [];
    },
  });
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-6">
        <Card className="p-5">
          ...
          {recentOut.length === 0 ? (
            <div className="text-sm text-muted-foreground py-8 text-center">{t("no_data")}</div>
          ) : (
            <div className="space-y-2">
              {recentOut.map((r: any) => (
                <div
                  key={r.id}
                  className="flex items-center justify-between text-sm border-t pt-2 first:border-0 first:pt-0"
                >
                  <div className="min-w-0">
                    <div className="font-medium truncate">{r.products?.nama_barang}</div>
                    <div className="text-xs text-muted-foreground">
                      {fmtDateTime(r.tanggal_keluar)} · {r.jenis_keluar}
                    </div>
                  </div>
          ...
          >
            {t("see_more")} <ArrowRight className="size-3.5" />
          </Link>
        </Card>
              ...
              {expiring.map((p: any) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <Td>
                      <Badge variant={st === "critical" ? "destructive" : "warning"}>
                        {t("expiry_warn")} {fmtDate(p.tanggal_kadaluarsa)}
                      </Badge>
                    </Td>
                    <Td>{p.stok}</Td>
                  </tr>
